...
# 3. Force DAG discovery
echo "🔄 Forcing DAG reserialization..."
AIRFLOW__LOGGING__LOGGING_LEVEL=WARNING airflow dags reserialize

# 4. Check DAG is loaded using JSON output
...
# 5. Unpause DAG using JSON to check result
echo "🔓 Unpausing DAG..."
...
# 7. Poll for completion using JSON + jq
...
while [ $attempt -lt $MAX_ATTEMPTS ]; do
    ...
    LATEST_STATE=$(airflow dags list-runs hello_world_dag -o json 2>/dev/null |
        jq -r '.[0].state // "no_runs"' 2>/dev/null || echo "no_runs")
    ...
    if [ "$LATEST_STATE" = "success" ]; then
        ...
        echo "📊 Run details:"
        airflow dags list-runs hello_world_dag -o json |
            jq -r '.[0] | "Run ID: \(.run_id)\nState: \(.state)\nStart: \(.start_date)\nEnd: \(.end_date)\nDuration: \(.run_duration // "N/A")s"'
